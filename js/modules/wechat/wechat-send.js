...
${memoryText}`;
            ...
            // 当日总结注入（如果开关开启）
            if (settings.dailySummaryInjectEnabled) {
                try {
                    const dailySummaryText = buildDailySummaryInjection(assistant.id);
                    if (dailySummaryText) {
                        systemPart += '\n\n' + dailySummaryText;
                        console.log('当日总结已注入聊天上下文');
                    }
                } catch (err) {
                    console.warn('当日总结注入失败，跳过:', err.message);
                }
            }
